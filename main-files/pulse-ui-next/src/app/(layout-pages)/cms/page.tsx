...
export default function CMSPage() {
  ...
  const fetchContent = async () => {
    ...
    try {
      setLoading(true);
      const url = `/content?mainPage=${selectedPage === 'business' ? 'Business Verticals' : selectedPage}&subSection=${selectedSubpage}&category=${selectedSection.label}`;
      const data = await api.get(url);
      
      if (data) {
        setContentDoc(data);
        // Sync local forms
        if (data.type === 'hero') setHeroForm(data.content);
        if (data.type === 'text') setTextForm(data.content);
      } else {
        setContentDoc(null);
        // Reset forms
  ...
  const handleSaveSection = async () => {
    ...
    try {
      let content = {};
      if (selectedSection.type === 'hero') content = heroForm;
      if (selectedSection.type === 'text') content = textForm;
      if (selectedSection.type === 'cards' || selectedSection.type === 'news') content = contentDoc?.content || [];
              ...
              <div key={page.id}>
                <button
                  ...
                  className={`w-full flex items-center justify-between p-3 rounded-xl transition-all ${
                    selectedPage === page.id && !selectedSubpage 
          ...
          <div className="flex items-center gap-6 overflow-x-auto no-scrollbar">
            {sections.map(section => (
              <button
                ...
                className={`flex flex-col items-center justify-center min-w-[100px] h-12 rounded-lg transition-all ${
